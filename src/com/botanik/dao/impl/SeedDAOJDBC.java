...
public class SeedDAOJDBC implements SeedDAO {
    ...
    public List<SeedModel> seedByModel(SeedModel model) {
        ...
        final String SQL_SEED_MODEL_DATA_BY_MODEL
                = //"select * from plant pl where pl.description =?" ;
...
//            conn.setAutoCommit(true);
            ...
            if (rs.next()) {
                model = new SeedModel();
                SeedWeight sw = new SeedWeight(rs.getBigDecimal("id"), rs.getString("weight_1000"), rs.getString("weight_100"), 
                        rs.getBigDecimal("type_id"), rs.getString("fruit_weight"),
...
//            conn.setAutoCommit(true);
            ...
            if (rs.next()) {
                ...
                SeedWeight sw = new SeedWeight(rs.getBigDecimal("id"), rs.getString("weight_1000"), rs.getString("weight_100"), 
                        rs.getBigDecimal("type_id"), rs.getString("fruit_weight"),
                        rs.getString("perfruit"));
                SeedMorphology sm = new SeedMorphology(rs.getBigDecimal("mor_id"), rs.getString("size"), rs.getBigDecimal("mor_type_id"), rs.getString("length"),
                        rs.getString("width"), rs.getString("height"), rs.getBigDecimal("aids"), rs.getString("configuration"), rs.getBigDecimal("embrio"));
                HerbariumSpecimenBase hs = new HerbariumSpecimenBase(rs.getBigDecimal("her_id"), rs.getString("number"), rs.getBigDecimal("location"), 
                        rs.getBigDecimal("dublicated"),
                        rs.getBoolean("isSpecimen"), rs.getString("voucher"));
                SamplingDataBase sd = new SamplingDataBase(rs.getBigDecimal("sam_id"), rs.getString("numbersample"), rs.getString("numberFood"),
                        rs.getString("area"), rs.getString("percentage"), rs.getString("note"));

                HabitatDataBase hd = new HabitatDataBase(rs.getBigDecimal("hab_id"), rs.getBigDecimal("habitat"), rs.getBigDecimal("landform"), rs.getString("geology"),
                        rs.getBigDecimal("aspect"), rs.getBigDecimal("factor"), rs.getBigDecimal("soil"), 
                        rs.getString("associated"), rs.getBigDecimal("slope"), rs.getBigDecimal("landuse"), rs.getString("site_notes"));

                LocationDataBase ld = new LocationDataBase(rs.getBigDecimal("loc_id"), rs.getBigDecimal("country_id"), rs.getBigDecimal("state_id"), rs.getString("latitude"),
                ...
                CollectionDataBase cd = new CollectionDataBase(rs.getBigDecimal("col_id"), rs.getString("col_number"), rs.getString("col_name"), rs.getString("access_number"),
                        rs.getDate("col_date"), rs.getBigDecimal("org_ins_id"), rs.getBigDecimal("wild_cult_id"), rs.getBigDecimal("col_name_ins_id"));

                Plant pl = new Plant(rs.getBigDecimal("plant_id"), rs.getBigDecimal("familyId"), rs.getBigDecimal("genusId"), 
                        rs.getBigDecimal("speciesId"), rs.getBigDecimal("speciesAuthorId"), rs.getBigDecimal("rankId"),
                        rs.getBigDecimal("infraSpeciesId"), rs.getBigDecimal("infraAuthorId"), rs.getBigDecimal("rank2Id"), 
                        rs.getBigDecimal("identifationStatusId"), rs.getBigDecimal("identifierInstituteId"),
                        rs.getString("plant_desc"), rs.getBigDecimal("usesId"), rs.getBigDecimal("vernacularNameId"),
                        rs.getBigDecimal("assesInforId"), rs.getBigDecimal("otherKeyId"), rs.getBigDecimal("plantFromId"), rs.getBigDecimal("infraSpecies2Id"),
                        rs.getBigDecimal("infraAuthor2Id"), rs.getBigDecimal("identifiedFromId"), rs.getDate("identificationDate"), rs.getString("plant_height"));
...
//            if (model.getPlant().getId() == null) {
//                model.getPlant().setId(-1);
//            }
//            if (model.getCollection().getId() == null) {
//                model.getCollection().setId(-1);
//            }
//            if (model.getLocation().getId() == null) {
//                model.getLocation().setId(-1);
//            }
//            if (model.getSampling().getId() == null) {
//                model.getSampling().setId(-1);
//            }
//            if (model.getHerbarium().getId() == null) {
//                model.getHerbarium().setId(-1);
//            }
//            if (model.getHabitat().getId() == null) {
//                model.getHabitat().setId(-1);
//            }
//            if (model.getMorphology().getId() == null) {
//                model.getMorphology().setId(-1);
//            }
//            if (model.getSeedWeight().getId() == null) {
//                model.getSeedWeight().setId(-1);
//            }

            Object[] values = {model.getPlant().getId(),
                model.getCollection().getId(),
                model.getLocation().getId(),
                model.getSampling().getId(),
                model.getHerbarium().getId(),
                model.getHabitat().getId(),
                model.getMorphology().getId(),
                model.getSeedWeight().getId(), new Date()};
            ps = prepareStatement(conn, SAVE_SEED, true, values);
            ps.execute();
            rs = ps.getGeneratedKeys();
            int gen_id = rs.next() ? rs.getInt(1) : 0;
...
//            daoUtils.getLogger().error(e);
    ...
    public void update(DefaultSeed model) {
        System.out.println("started DefaultSeed " + model);
        PreparedStatement ps = null;
        Connection conn = null;
        final String UPDATE_SEED = "update seed  set plant_id =?, collection_id =? , location_id=? , sampling_id =? , herbarium_id = ?, habitat_id =? ,"
                + "seed_mor_id =? , seed_weight_id =?  where id= ?";
...
//            if ((model.getPlant().getId() == null) || (model.getPlant().getId() == 0)){
//                model.getPlant().setId(-1);
//            }
//            if (model.getCollection().getId() == null) {
//                model.getCollection().setId(-1);
...
//            }
            ...
            System.out.println("Object " + Arrays.toString(values));
            ps = prepareStatement(conn, UPDATE_SEED, values);
            ps.execute();
//            model.setDaoStatus("1");
        } catch (SQLException e) {
...
//                model = new SeedWeightView();
                model = new SeedWeightView(rs.getBigDecimal("id"), rs.getString("weight_1000"), rs.getString("weight_100"),
...
//            conn.setAutoCommit(true);
            ...
            if (rs.next()) {
                ...
                model = new SeedMorphologyView(rs.getBigDecimal("mor_id"), rs.getString("size"), rs.getString("type"), rs.getString("length"),
...
//            conn.setAutoCommit(true);
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                model = new SamplingDataView(rs.getBigDecimal("sam_id"), rs.getString("numbersample"), rs.getString("numberFood"),
                        rs.getString("area"));
