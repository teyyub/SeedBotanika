...
public class SearchDAOJDBC implements SearchDAO {
    ...
    public List<Search> search(SearchModel model) {
        Logger.save("started List<Search> search(SearchModel model)");
        ResultSet rs = null;
...
//        }    
        ...
        try {
            ...
            Logger.save("exception in SQL_SEARCH_MODEL_DATA_BY_MODEL " + e.toString());
        } catch (IOException | ClassNotFoundException ex) {
            Logger.save(ex.getMessage());
        } finally {
            close(conn, ps, rs);
        }
        Logger.save("search model list size " + modelList.size());
        return modelList;
    }
